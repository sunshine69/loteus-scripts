...
build_external_module() {
    ...
    #for kmoddir in bcwc_pcie; do
		if [ ! -d /mnt/portdata/tmp/$kmoddir ]; then continue; fi
...
sudo bash -c "KBUILDDIR_ENV=$(pwd) KVERS=\"$KVER\" $SCRIPT_DIR/rebuild-initrd.sh \"\" \"$(pwd)/initrd.xz\""
...
if [ -d $TARGET_DIR/publish ]; then mv $TARGET_DIR/porteus-kernel-$KVER.tar.sfx $TARGET_DIR/publish/porteus-kernel-$KVER.tar.sfx; echo moved to publish dir; fi
